...
class c_Int_Construct: public c_Base_Construct
{
protected:
...
public:
     ...
     c_Int_Construct(c_Table_Handler * p_Tables, string p_Assembly_Name, string p_Construct_Name, int p_CID, int p_Type = 0)
     {
          cout << "\n\t c_Int_Construct()";
     ...
     void update_Settings()
     {
          ...
          flg_Table_Output[5] = Settings_Table->get_Data_Bit_At_Given_Index_In_Given_Cell_In_Given_Row_F(15, 1, 0);
     }
     ...
     void eval_Input_Row(int p_Row)
     {
          ...
          {
               tmp_TRID = Node_Tree.get_TRID(str2int(Input_Table->get_string(p_Row, cou_Index)));
     ...
     {
          c_Num_Bin_Node_Int * tmp_Node = NULL;
          u_Data tmp_Data;

          //Eval all the cells in the given row..
          for (int cou_Index=0;cou_Index<Input_Table->get_Row_Cell_Count(p_Row);cou_Index++)
          {
               int tmp_Input = str2int(Input_Table->get_string(p_Row, cou_Index));
     ...
     void discharge_Treetop(int p_TRID, int p_Row, int p_Oflg = 2)
     {
          ...
          for (int cou_Index=tmp_Low;cou_Index<=tmp_High;cou_Index++)
          {
               ...
               if (tmp_TRID_Related_Value == 0.0){ Output_Table_Charge->push_int(cou_Index, 0.0); continue; }
               ...
               if (flg_Table_Output[0] == p_Oflg || flg_Table_Output[0] == 3){ Output_Table_Pattern->pushc_string(int2str(tmp_TRID_Related_Value)); }
               ...
               if (flg_Table_Output[3] == p_Oflg || flg_Table_Output[3] == 3)
               {
                    ...
                    float tmp_Truth = str2int(Input_Table_Truth->get_string(p_Row, 0));
               ...
               {
                    //cout << "\n\t\t cou_Index->" << cou_Index;
                    //cout << "\n\n\t\t\t score = " << Node_Tree.get_RC_Score(cou_Index) << " / " << RC_Average << " = " << (Node_Tree.get_RC_Score(cou_Index) / RC_Average);
     ...
     //==--  These members are for gathering some output tables.
     ...
     void gather_Output_Tables()
     {
          ...
          for (int cou_Row=0;cou_Row<Output_Table_Pattern->get_Row_Count();cou_Row++)
          {
               ...
               for (int cou_Cell=0;cou_Cell<Output_Table_Pattern->get_Row_Cell_Count(cou_Row);cou_Cell++)
               {
                    ...
                    tmp_Pattern.I = str2int(Output_Table_Pattern->get_string(cou_Row, cou_Cell));
                    tmp_TRID = Node_Tree.get_TRID(tmp_Pattern.I);
                    ...
                    {
                         cout << "\t Truth";
                         float tmp_Low_Tolerance = tmp_Pattern.I - Tolerance;
                         float tmp_High_Tolerance = tmp_Pattern.I + Tolerance;

                         float tmp_Truth = str2int(Input_Table_Truth->get_string(cou_Row, 0));
               ...
               if (flg_Table_Output[5] > 0)
               {
                    ...
                    Output_Table_Vision->set_int(cou_Row, 0, tmp_Vision);
               }
          }
     ...
     virtual void initiate_Backpropagation_Foreign_Node(u_Data p_Data, int p_Row, int p_Charge)
     {
          ...
          c_Num_Bin_Node_Int * tmp_Node = Treetop_Bin_Tree.exists(p_Data);

          if (tmp_Node == NULL)
          {
               Output_Table_Pattern->push_string(p_Row, "0");
               return;
          }

          //Gather the backpropogated pattern using the current cell in the given row.
          Output_Table_Pattern->push_string(p_Row, int2str(tmp_Node->Pattern));
     }
     ...
     string get_Stats()
     {
          return " C.i:" + Construct_Name + " #N:" + int2str(Node_Tree.get_Row_Count());
     }
