...
public:
     ...
     c_Base_CAN_1D(c_Base_Node_Network_1D * p_Node_Network, c_Charging_Buffer_1D * p_Charging_Buffer, c_Table_1D * p_Treetop_Table, ofstream * p_BenchMark)
     {
          //The node network.
     ...
     {
          cout << "\n charge_Tier(" << p_Tier << ", " << p_Charge << " = 1000) Node_Count->" << (Number_Of_Tiers - p_Tier);
          ...
          for (int cou_NID=0;cou_NID<(Number_Of_Tiers - p_Tier);cou_NID++)
          {
               cout << "\n\n\t cou_NID->" << cou_NID; cout.flush();
               cout << "\n\t   ~" << CAN_Nodes[p_Tier] [cou_NID] << "~"; cout.flush();
               cout << " CB->" << Charging_Buffer; cout.flush();
               cout << " Can_Nodes[" << p_Tier << "] [" << cou_NID << "]->" << CAN_Nodes[p_Tier] [cou_NID]; cout.flush();
               cout << " p_Charge->" << p_Charge; cout.flush();
               Charging_Buffer->output_Charging_Buffer();
               Charging_Buffer->submit(CAN_Nodes[p_Tier] [cou_NID], p_Charge, 0);
               cout << "\n\t  SUBMISSION COMPLETE..."; cout.flush();
               //Charging_Buffer->Tree.output_Tree();
          }
     }
