...
public:
     ...
     //Contains -
     c_Num_Bin_Tree_Int Bin_Tree[1000];
     ...
     c_Num_Bin_Node_Int * submit_Input(int p_Data)
     {
          int tmp_FDF = gf3di(p_Data);
     ...
     c_Num_Bin_Node_Int * submit_Input_RC(int p_Data)
     {
          int tmp_FDF = gf3di(p_Data);
     ...
     int get_TRID(int p_Data)
     {
          int tmp_FDF = gf3di(p_Data);
          Bin_Tree[tmp_FDF].search(p_Data);
          return Bin_Tree[tmp_FDF].get_Current_Node_TRID();
     }
     
     //Gets the pattern from the table for the given row (TRID).
     int get_Pattern(int p_Index)
     {
          return Table.get_int(p_Index, 0);
     }
     
     float get_RC_Score(int p_Index)
     {
          int tmp_Pattern = get_Pattern(p_Index);
          int tmp_FDF = gf3di(tmp_Pattern);
     ...
     void sort_Nodes_Int()
     {
          ...
          {
               tmp_Pattern = Table.get_int(cou_Index, 0);
