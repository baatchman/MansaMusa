//This class is used by the node network to house the nodes on the tiers, helps to avoid large allocations of data.
class c_Base_Node_Linked_List
{
public:
     
     //The next node in the list.
     c_Base_Node_Linked_List * Next;
     
     //The node contained in the linked list node.
     c_Base_Node_1D * NID;
     
     c_Base_Node_Linked_List()
     {
          Next = NULL;
     ...
     c_Base_Node_Linked_List(c_Base_Node_1D * p_NID)
     {
          NID = p_NID;
     }
...
public:
     ...
     //c_Base_Node_1D *** Nodes;
     
     //This is the linked list that holds all of the Nodes. Replaces the
     //MD array of nodes. The first index is the tier.
     c_Base_Node_Linked_List ** Nodes;
     
     //This keeps track of the current ending node on the tier.
     c_Base_Node_Linked_List ** Current_LL_Node;
     ...
     c_Fractal_State_Bin_Tree ** Treetop_Bin_Tree;
     ...
     c_Base_Node_Network_1D()
     {
          //Create the array to hold the nodes.
          Nodes = new c_Base_Node_Linked_List*[1];
          Nodes[0] = NULL;
          
          //Current node in each tier.
          Current_LL_Node = new c_Base_Node_Linked_List*[1];
          Current_LL_Node[0] = Nodes[0];
          ...
          *Node_Count_MAX[0] = 0;
          ...
          Number_Of_Tiers = 0;
          Number_Of_Tiers_MAX = 0;
          
          Current_TtID.I = 0;
          Treetop_Bin_Tree = new c_Fractal_State_Bin_Tree*[1];
          Treetop_Bin_Tree[0] = new c_Fractal_State_Bin_Tree;
     ...
     //--     NODE CREATION FOR LOADING, IJECTS INTO GIVEN INDEX. DOES NOT NOW!
     //We are running with no bounds checking as only the loading functions should be using.
     //As long as the nodes are sequentially loaded they should be placed in the correct order.
     
     //Creates a new node on the given tier.
     virtual c_Base_Node_1D * new_Node(int p_Tier, int p_Index)
     {
          c_Base_Node_1D * tmp_Node = NULL;
          
          //Create the new node and set the current to the next one in the linked list.
          Current_LL_Node[p_Tier]->NID = new c_Simple_Node;
          Current_LL_Node[p_Tier]->NID->set_Index(p_Index);
          tmp_Node = Current_LL_Node[p_Tier]->NID;
          
          Current_LL_Node[p_Tier] = Current_LL_Node[p_Tier]->Next;
     ...
     //Creates a new node on the given tier.
     virtual c_Base_Node_1D * new_RC_Node(int p_Tier, int p_Index)
     {
          c_Base_Node_1D * tmp_Node = NULL;
          
          //Create the new node and set the current to the next one in the linked list.
          Current_LL_Node[p_Tier]->NID = new c_RC_Node_1D;
          Current_LL_Node[p_Tier]->NID->set_Index(p_Index);
          tmp_Node = Current_LL_Node[p_Tier]->NID;
     ...
     //Creates a new node on the given tier.
     virtual c_Base_Node_1D * new_Treetop_Node(int p_Tier, int p_Index)
     {
          c_Base_Node_1D * tmp_Node = NULL;
          ...
          //Gather the current node so that the Current_LL_Node can be set to the next node in the chain but the current node can still be manipulated and returned.
          tmp_Node = Current_LL_Node[p_Tier]->NID;
          Current_LL_Node[p_Tier] = Current_LL_Node[p_Tier]->Next;
          
          tmp_Node->set_Index(p_Index);
          ...
          Treetop_Bin_Tree[int (Current_TtID.I / 1000)]->set_Current_Node_NID(tmp_Node);
          
          //Sets the state for the node to the identifier.
          tmp_Node->set_Treetop(Current_TtID);
          
          //Sets the U_CID, Tier, and Index;
          tmp_Node->set_CID(CID);
          tmp_Node->set_Tier(p_Tier);
          tmp_Node->set_Index(p_Index);
     ...
     //Creates a new node on the given tier.
     virtual c_Base_Node_1D * new_Treetop_RC_Node(int p_Tier, int p_Index)
     {
          c_Base_Node_1D * tmp_Node = NULL;
          
          //Create the new node and set the current to the next one in the linked list.
          Current_LL_Node[p_Tier]->NID = new c_Treetop_RC_Node_1D;
     ...
     virtual c_Base_Node_1D * new_State_Node(int p_Index, u_Data p_State)
     {          
          c_Base_Node_1D * tmp_Node = NULL;
          
          //Create the new node and set the current to the next one in the linked list.
          Current_LL_Node[0]->NID = new c_State_Node_1D;
          
          //Gather the current node so that the Current_LL_Node can be set to the next node in the chain but the current node can still be manipulated and returned.
          tmp_Node = Current_LL_Node[0]->NID;
          Current_LL_Node[0] = Current_LL_Node[0]->Next;
          
          tmp_Node->set_Index(p_Index);
          tmp_Node->set_State(p_State);
          
          return tmp_Node;
     }
     
     //Creates a state node with the given state.
     virtual c_Base_Node_1D * new_RC_State_Node(int p_Index, u_Data p_State)
     {
          c_Base_Node_1D * tmp_Node = NULL;
          
          //Create the new node and set the current to the next one in the linked list.
          Current_LL_Node[0]->NID = new c_RC_State_Node_1D;
     ...
     //Creates a state node with the given state.
     virtual c_Base_Node_1D * new_Treetop_State_Node(int p_Tier, int p_Index, u_Data p_State)
     {
          c_Base_Node_1D * tmp_Node = NULL;
          
          //Create the new node and set the current to the next one in the linked list.
          Current_LL_Node[p_Tier]->NID = new c_Treetop_State_Node_1D;
     ...
     //Creates a state node with the given state.
     virtual c_Base_Node_1D * new_Treetop_RC_State_Node(int p_Tier, int p_Index, u_Data p_State)
     {
          c_Base_Node_1D * tmp_Node = NULL;
          
          //Create the new node and set the current to the next one in the linked list.
          Current_LL_Node[p_Tier]->NID = new c_Treetop_RC_State_Node_1D;
     ...
     //Creates a new node on the given tier.
     virtual c_Base_Node_1D * new_Node(int p_Tier)
     {
          ...
          tmp_Node->set_Index(*Node_Count[p_Tier] - 1);
          
          //Increment the Node_Count now that we do not use the incement tier size.
          *Node_Count[p_Tier]++;
     ...
     //Creates a new node on the given tier.
     virtual c_Base_Node_1D * new_RC_Node(int p_Tier)
     {
          //If the tier does not yet exist then create it by expanding the arrays as necessary.
          if (p_Tier >= Number_Of_Tiers)
          {
     ...
     //Creates a new node on the given tier.
     virtual c_Base_Node_1D * new_Treetop_Node(int p_Tier)
     {
          //If the tier does not yet exist then create it by expanding the arrays as necessary.
          if (p_Tier >= Number_Of_Tiers)
          {
          ...
          //Create the new node and set the current to the next one in the linked list.
          Current_LL_Node[p_Tier]->NID = new c_Treetop_Simple_Node_1D;
     ...
     //Creates a new node on the given tier.
     virtual c_Base_Node_1D * new_Treetop_RC_Node(int p_Tier)
     {
          //If the tier does not yet exist then create it by expanding the arrays as necessary.
          if (p_Tier >= Number_Of_Tiers)
          {
     ...
     //Creates a state node with the given state.
     virtual c_Base_Node_1D * new_State_Node(u_Data p_State)
     {
          //If the tier does not yet exist then create it by expanding the arrays as necessary.
          ...
          tmp_Node->set_Index(*Node_Count[0] - 1);
     ...
     //Creates a state node with the given state.
     virtual c_Base_Node_1D * new_RC_State_Node(u_Data p_State)
     {
          //If the tier does not yet exist then create it by expanding the arrays as necessary.
          if (Number_Of_Tiers < 1)
          {
     ...
     //Creates a state node with the given state.
     virtual c_Base_Node_1D * new_Treetop_State_Node(int p_Tier, u_Data p_State)
     {
          //If the tier does not yet exist then create it by expanding the arrays as necessary.
          if (p_Tier >= Number_Of_Tiers)
          {
     ...
     //Creates a state node with the given state.
     virtual c_Base_Node_1D * new_Treetop_RC_State_Node(int p_Tier, u_Data p_State)
     {
          //If the tier does not yet exist then create it by expanding the arrays as necessary.
          if (p_Tier >= Number_Of_Tiers)
          {
     ...
     //Gets a state node.
     virtual c_Base_Node_1D * get_State_Node(u_Data p_State, int p_Tree = 0)
     {
     ...
     //Gets a state node.
     virtual c_Base_Node_1D * get_RC_State_Node(u_Data p_State, int p_Tree = 0)
     {
          //increase_State_Bin_Tree(p_Tree);
     ...
     //Gets a state node.
     virtual c_Base_Node_1D * get_Treetop_State_Node(u_Data p_State, int p_Tree = 0)
     {
          //increase_State_Bin_Tree(p_Tree);
     ...
     //Gets a state node.
     virtual c_Base_Node_1D * get_Treetop_RC_State_Node(u_Data p_State, int p_Tree = 0)
     {
          //increase_State_Bin_Tree(p_Tree);
     ...
     virtual c_Base_Node_1D * get_Treetop_Node(u_Data p_State)
     {
     ...
     virtual void set_Treetop_Node(u_Data p_State, c_Base_Node_1D * p_Node)
     {    
     ...
     virtual c_Base_Node_1D * get_Treetop_Node_Reference(u_Data p_Handle)
     {
     ...
     c_Base_Node_1D * get_Treetop_Connection(int p_Upper_Tier, c_Base_Node_1D * p_Left, c_Base_Node_1D * p_Right)
     {
          //Temporary node to hold the connection when found.
          c_Base_Node_1D * tmp_Upper_Tier_Connection;
     ...
     //Gets the upper tier connectino between the given nodes, if none found one is created and returned, this is for treetops.
     c_Base_Node_1D * get_Treetop_RC_Connection(int p_Upper_Tier, c_Base_Node_1D * p_Left, c_Base_Node_1D * p_Right)
     {
          //Temporary node to hold the connection when found.
          c_Base_Node_1D * tmp_Upper_Tier_Connection;
     ...
     c_Base_Node_1D * create_Treetop_Connection(int p_Upper_Tier, c_Base_Node_1D * p_Left, c_Base_Node_1D * p_Right)
     {
          //Create the upper tier node to connect to.
          c_Base_Node_1D * Upper_Tier_Node = new_Treetop_Node(p_Upper_Tier);
     ...
     c_Base_Node_1D * create_Treetop_RC_Connection(int p_Upper_Tier, c_Base_Node_1D * p_Left, c_Base_Node_1D * p_Right)
     {
          //Create the upper tier node to connect to.
          c_Base_Node_1D * Upper_Tier_Node = new_Treetop_RC_Node(p_Upper_Tier);
     ...
     //Creates an upper tier connection.
     c_Base_Node_1D * create_Upper_Tier_Connection(int p_Upper_Tier, c_Base_Node_1D * p_Left, c_Base_Node_1D * p_Right)
     {
          //Create the upper tier node to connect to.
          c_Base_Node_1D * Upper_Tier_Node = new_Node(p_Upper_Tier);
     ...
     c_Base_Node_1D * does_Upper_Tier_Connection_Exist(c_Base_Node_1D * p_Left, c_Base_Node_1D * p_Right)
     {
     ...
     c_Base_Node_1D * get_Upper_Tier_Connection(int p_Upper_Tier, c_Base_Node_1D * p_Left, c_Base_Node_1D * p_Right)
     {
          //Temporary node to hold the connection when found.
          c_Base_Node_1D * tmp_Upper_Tier_Connection;
     ...
     //Creates an upper tier connection.
     c_Base_Node_1D * create_Upper_Tier_Connection_RC(int p_Upper_Tier, c_Base_Node_1D * p_Left, c_Base_Node_1D * p_Right)
     {
          //Create the upper tier node to connect to.
          c_Base_Node_1D * Upper_Tier_Node = new_RC_Node(p_Upper_Tier);
     ...
     //Returns true of false depending on if an upper tier connection already exists.
     c_Base_Node_1D * does_Upper_Tier_Connection_Exist_RC(c_Base_Node_1D * p_Left, c_Base_Node_1D * p_Right)
     {
          //If either of them have no axons then the answer is no.
          if (!p_Left->get_Axon_Count(0) || !p_Right->get_Axon_Count(1))
          {
     ...
     //Gets the upper tier connectino between the given nodes, if none found one is created and returned.
     c_Base_Node_1D * get_Upper_Tier_Connection_RC(int p_Upper_Tier, c_Base_Node_1D * p_Left, c_Base_Node_1D * p_Right)
     {
          //Temporary node to hold the connection when found.
          c_Base_Node_1D * tmp_Upper_Tier_Connection;
     ...
     c_Base_Node_1D * create_Chrono_Connection(c_Base_Node_1D * p_Previous, c_Base_Node_1D * p_Lower, int p_Tier)
     {          
          //Create the chrono tier node to connect to.
          c_Base_Node_1D * Chrono_Node = new_Treetop_Node(p_Tier);
     ...
     c_Base_Node_1D * does_Chrono_Connection_Exist(c_Base_Node_1D * p_Previous_Node, c_Base_Node_1D * p_Lower)
     {
          //If either of them have no axons then the answer is no.
     ...
     c_Base_Node_1D * get_Chrono_Connection(c_Base_Node_1D * p_Previous, c_Base_Node_1D * p_Lower, int p_Tier = 1)
     {
          //Temporary node to hold the connection when found.
          c_Base_Node_1D * tmp_Chrono_Connection;
     ...
     //Creates a chrono connection.
     c_Base_Node_1D * create_Chrono_Connection_RC(c_Base_Node_1D * p_Previous, c_Base_Node_1D * p_Lower, int p_Tier)
     {          
          //Create the chrono tier node to connect to.
          c_Base_Node_1D * Chrono_Node = new_Treetop_RC_Node(p_Tier);
     ...
     //Returns true or false depending upon whether or not the current chrono node exists.
     c_Base_Node_1D * does_Chrono_Connection_Exist_RC(c_Base_Node_1D * p_Previous_Node, c_Base_Node_1D * p_Lower)
     {
          //If either of them have no axons then the answer is no.
          if (!p_Previous_Node->get_Axon_Count(0) || !p_Lower->get_Axon_Count(1))
          {
     ...
     //Gets the chrono node linking the queried nodes, if none are found it creates one and returns it instead.
     c_Base_Node_1D * get_Chrono_Connection_RC(c_Base_Node_1D * p_Previous, c_Base_Node_1D * p_Lower, int p_Tier = 1)
     {
          //Temporary node to hold the connection when found.
          c_Base_Node_1D * tmp_Chrono_Connection;
     ...
     void create_Connection(int p_Leg, c_Base_Node_1D * p_From, c_Base_Node_1D * p_To)
     {
     ...
     void increase_Treetop_Bin_Tree(int p_Chunk)
     {
          ...
          c_Fractal_State_Bin_Tree ** tmp_Treetop_Bin_Tree;
          tmp_Treetop_Bin_Tree = new c_Fractal_State_Bin_Tree*[tmp_Size];
          ...
          Treetop_Bin_Tree = new c_Fractal_State_Bin_Tree*[p_Chunk];
          
          //Allocate the new tree.
          Treetop_Bin_Tree[p_Chunk -1] = new c_Fractal_State_Bin_Tree;
     ...
     */
     ...
     void increase_Number_Of_Tiers(int p_Tier)
     {
          ...
          c_Base_Node_Linked_List ** tmp_Nodes = new c_Base_Node_Linked_List*[Number_Of_Tiers];
     ...
     void initiate_Backpropagation_Output_Only(c_Base_Node_1D * &p_Node)
     {
     ...
     void initiate_Backpropagation(c_Base_Node_1D * &p_Node)
     {
          //For when directly output the dendrites of a nodes on the base tier.
          if (p_Node == NULL){ return; }
     ...
     void initiate_Backpropagation_LEFT(c_Base_Node_1D * &p_Node)
     {
     ...
     void initiate_Backpropagation_RIGHT(c_Base_Node_1D * &p_Node)
     {
     ...
     //Initiates a backpropagation LEFT.
     void initiate_Backpropagation_LEFT_Output_Only(c_Base_Node_1D * &p_Node)
     {
          //If a left leg exists then initiate a backpropagation along it, then along the right side.
          if (p_Node->Dendrites[0] != NULL)
          {
     ...
     //Initiates a backpropagation RIGHT
     void initiate_Backpropagation_RIGHT_Output_Only(c_Base_Node_1D * &p_Node)
     {
          //If a right leg exists then initiate a backpropagation.
          if (p_Node->Dendrites[1] != NULL)
          {
     ...
     //Initiates a backpropagation in the node network.
     void initiate_Backpropagation_Gather_Data(c_Base_Node_1D * &p_Node, c_Cell_1D * p_Cell)
     {
     ...
     //Initiates a backpropagation LEFT.
     void initiate_Backpropagation_LEFT_Gather_Data(c_Base_Node_1D * &p_Node, c_Cell_1D * p_Cell)
     {
          //If a left leg exists then initiate a backpropagation along it, then along the right side.
          if (p_Node->Dendrites[0] != NULL)
          {
     ...
     //Initiates a backpropagation RIGHT
     void initiate_Backpropagation_RIGHT_Gather_Data(c_Base_Node_1D * &p_Node, c_Cell_1D * p_Cell)
     {
          //If a right leg exists then initiate a backpropagation.
          if (p_Node->Dendrites[1] != NULL)
          {
     ...
     void initiate_Backpropagation_From_Given_u_Data_For_Given_Row(u_Data p_Data_Node, int p_Row)
     {
          ...
          c_Base_Node_1D * tmp_Node = get_Treetop_Node(p_Data_Node);
     ...
     void initiate_Backpropagation_For_Given_Row(c_Base_Node_1D * &p_Node, int p_Row)
     {
          //For when directly output the dendrites of a nodes on the base tier.
          if (p_Node == NULL){ return; }
     ...
     //Initiates a backpropagation LEFT.
     void initiate_Backpropagation_LEFT_For_Given_Row(c_Base_Node_1D * &p_Node, int p_Row)
     {
          //If a left leg exists then initiate a backpropagation along it, then along the right side.
          if (p_Node->Dendrites[0] != NULL)
          {
     ...
     //Initiates a backpropagation RIGHT
     void initiate_Backpropagation_RIGHT_For_Given_Row(c_Base_Node_1D * &p_Node, int p_Row)
     {
          //If a right leg exists then initiate a backpropagation.
          if (p_Node->Dendrites[1] != NULL)
          {
     ...
     float get_RCA(int p_Tier = -1)
     {
          ...
          //The current tmp_Node;
          c_Base_Node_Linked_List * tmp_Node = Nodes[p_Tier];
          while (tmp_Node != NULL)
          {
               tmp_RCA += tmp_Node->NID->get_RC_Score();
     ...
     int get_RC_Score(c_Base_Node_1D * &p_Node)
     {
     ...
     {
          //Gather the address of the current treetop node.
          c_Base_Node_1D * tmp_Node = get_Treetop_Node(p_Data);
          
          return get_RC_Score(tmp_Node);
     }
     
     //Get the node RC for a given node.
     int get_RC(c_Base_Node_1D * &p_Node)
     {
          if (p_Node == NULL){ return 0.0; }
     ...
     //Get the node RC for a given node.
     int get_RC_LVL(c_Base_Node_1D * &p_Node)
     {
          if (p_Node == NULL){ return 0.0; }
     ...
     int get_Node_Index(c_Base_Node_1D * p_Node, int p_Tier)
     {
     ...
     void output_Node(int p_Tier, int p_NID)
     {
          ...
          while (tmp_Node != NULL)
          {
               ...
               if (tmp_Node == NULL){ cout << " NULL_NODE"; return; }
               
               //initiate_Backpropagation_Output_Only(Nodes[p_Tier] [p_NID]);
               //cout << " -" << Nodes[p_Tier] [p_NID];
               tmp_Node->NID->output_Node_Header();
               
               cout << " S->";
               initiate_Backpropagation_Output_Only(tmp_Node->NID);
               
               cout << " ~D";
               cout << " &";
               initiate_Backpropagation_Output_Only(tmp_Node->NID->Dendrites[0]);
               
               cout << " &";
               initiate_Backpropagation_Output_Only(tmp_Node->NID->Dendrites[1]);
               
               cout << " ~A[0]";
                    
               for (int cou_A=0;cou_A<tmp_Node->NID->get_Axon_Count(0);cou_A++)
               {
                    cout << " &";
                    initiate_Backpropagation_Output_Only(tmp_Node->NID->Axons[0] [cou_A]);
               }
                    
               cout << " ~A[1]";
               
               for (int cou_A=0;cou_A<tmp_Node->NID->get_Axon_Count(1);cou_A++)
               {
                    cout << " &";
                    initiate_Backpropagation_Output_Only(tmp_Node->NID->Axons[1] [cou_A]);
               }
     ...
     void output_Node_Network()
     {
          ...
          for (int cou_T=0;cou_T<Number_Of_Tiers;cou_T++)
          {
               ...
               //The current tmp_Node;
               c_Base_Node_Linked_List * tmp_Node = Nodes[cou_T];
               while (tmp_Node != NULL)
               {
                    ochr(0, (tmp_Node->NID->get_Reinforcement_Counter() + tmp_Node->NID->get_Reinforcement_Counter_Level()), 'X');
                    tmp_Node = tmp_Node->Next;
               }
          }
          cout << "\n";
     ...
     void Save(string p_Directory = "Default", string p_Assembly_Name = "")
     {
          ...
          //cout << "\n\n\t [" << 0 << "]";
          //The current tmp_Node;
          c_Base_Node_Linked_List * tmp_Node = Nodes[0];
          while (tmp_Node != NULL)
          {
               //cout << "\n\t  [" << 0 << "] [" << cou_Index << "]";
               ...
               SF << "\n" << 0 << " " << tmp_Node->NID->get_Index() << " ";
               SF << " " << tmp_Node->NID->get_Type();
               if (State_Type == 0)
               { 
                    SF << " SN"; 
                    SF << " " << (tmp_Node->NID->get_State()).I;
               }
               
               if (State_Type == 1)
               { 
                    SF << " SNR"; 
                    SF << " " << tmp_State_Coordinates[0] [tmp_Node->NID->get_Index()];
                    SF << " " << tmp_State_Coordinates[1] [tmp_Node->NID->get_Index()];
                    SF << " " << tmp_State_Coordinates[2] [tmp_Node->NID->get_Index()];
               }
               SF << " " << get_RC(tmp_Node->NID);
               SF << " " << get_RC_LVL(tmp_Node->NID);
               SF << " " << (tmp_Node->NID->get_Treetop()).I;
          ...
          for (int cou_Tier=1;cou_Tier<Number_Of_Tiers;cou_Tier++)
          {
               ...
               //The current tmp_Node;
               c_Base_Node_Linked_List * tmp_Node = Nodes[cou_Tier];
               ...
               while (tmp_Node != NULL)
               {
                    ...
                    SF << "\n" << cou_Tier << " " << tmp_Node->NID->get_Index() << " ";
                    SF << " " << tmp_Node->NID->get_Type();
                    SF << " M";
                    SF << " " << tmp_Node->NID->get_Reinforcement_Counter();
                    SF << " " << tmp_Node->NID->get_Reinforcement_Counter_Level();
                    
                    SF << " " << (tmp_Node->NID->get_Treetop()).I;
                    
                    SF << " " << get_Node_Index(tmp_Node->NID->Dendrites[0], (cou_Tier - 1));
                    SF << " " << get_Node_Index(tmp_Node->NID->Dendrites[1], (cou_Tier - 1));
     ...
     {    
		...
		int tmp_node_Y = get_Console_Cursor_Y();
          ...
          while (!SF.eof())
          {
               ...
               //cout << "\n\t tmp_Type->" << tmp_Type;
               if (tmp_Type == "State_Node"){ tmp_Node = new_State_Node(tmp_Index, tmp_State); }
               if (tmp_Type == "RC_State_Node"){ tmp_Node = new_RC_State_Node(tmp_Index, tmp_State); }
               if (tmp_Type == "Treetop_State_Node"){ tmp_Node = new_Treetop_State_Node(tmp_Tier, tmp_Index, tmp_State); }
               if (tmp_Type == "Treetop_RC_State_Node"){ tmp_Node = new_Treetop_RC_State_Node(tmp_Tier, tmp_Index, tmp_State); }
               
               if (tmp_Type == "Simple_Node"){ tmp_Node = new_Node(tmp_Tier, tmp_Index); }
               if (tmp_Type == "RC_Node"){ tmp_Node = new_RC_Node(tmp_Tier, tmp_Index); }
               if (tmp_Type == "Treetop_Simple_Node"){ tmp_Node = new_Treetop_Node(tmp_Tier, tmp_Index); }
               if (tmp_Type == "Treetop_RC_Node"){ tmp_Node = new_Treetop_RC_Node(tmp_Tier, tmp_Index); }
               ...
               if (tmp_T == "SN")
               {
                    ...
                    State_Tree.search(tmp_State); State_Tree.set_Current_Node_NID(tmp_Node);
                    
                    //cout << " tmp_State->" << tmp_State.I; //cout.flush();
                    tmp_Node->set_State(tmp_State);
               }
               if (tmp_T == "SNR")
               {
                    SF >> tmp_State_CID;
               ...
               tmp_Node->set_Reinforcement_Counter(tmp_RC);
               tmp_Node->set_Reinforcement_Counter_Level(tmp_RC_Lvl);
               ...
               SF >> tmp_Treetop.I;
               tmp_Node->set_Treetop(tmp_Treetop);
          ...
          for (int cou_T=1;cou_T<Number_Of_Tiers;cou_T++)
          {
               ...
               for (int cou_Index=0;cou_Index<*Node_Count[cou_T];cou_Index++)
               {
                    ...
                    //create_Connection(int, c_Base_Node_1D*, c_Base_Node_1D*)
