...
//                    int tmp_RC = ((c_Poly_Node*) CAN_Nodes[cou_T] [cou_Node]->Current_Target_Node)->get_Reinforcement_Counter();
     ...
     void build_Inputs(int p_Method)
     {
          ...
          if (p_Method == 1)
          {
               ...
               if (Construct_Type == 0)
               {
                    submit_CAN_Raw_Node_Array(tmp_Input_Data, tmp_Current_Index);
               }
               ...
               if (Construct_Type == 1)
               {
                    submit_CAN_Node_Reference_Array(tmp_Input_Data_Reference, tmp_Current_Index); 
               }
     ...
     void create_Settings_Table()
     {
          ...
          cout << "\n\t\t\t -Tables->set_Cell_Name( Construct_Type:, Settings_Table, 0);";
